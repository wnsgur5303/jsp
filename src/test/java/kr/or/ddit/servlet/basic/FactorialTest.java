...
//@Test : 테스트 메소드에 붙이는 이노테이션
	/*	해당 이노테이션을 붙이지 않으면 테스트 메소드로 인식되지 않음(테스트 대상에서 제외)
		제한사항
		...
		2. assert 구문이 있을 경우 assert구문이 전부 통과 했을떄(참으로 인정 되었을때)
		
	@Before : 초기화 메소드에 붙이는 이노테이션
		@Test 이노테이션이 붙은 텍스트 메소드를 호출하기전에 먼저 실행됨
		@Before ==> @Test (test)
		@Before ==> @Test (test2)
		@Before ==> @Test (test3)
	...
	*/
	...
	public void test4() {
		...
		/***When***///테스트를 검증하고자 하는 행위(mathod call)
		int result = factorial.calculate(4);
